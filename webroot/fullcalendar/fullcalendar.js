...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		return '<a id="book_appoi'+event.id+'" main="'+ event.id +'" main1="'+ event.food_date +'" class="book_app ' + classes.join(' ') + '"' +
				(event.url ?
...
TimeGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		return '<a main="'+ btoa(event.id) +'" class="' + classes.join(' ') + ' book_slots"' + 
			(event.url ?
				' href="' + htmlEscape(event.url) + '"' :
				''
				) +
