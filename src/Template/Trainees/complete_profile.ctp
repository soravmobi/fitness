...
<!-- Image Profile With Crop Start -->

<script type="text/javascript">
    /*$(document).ready(function(){
      $('#trainee_profile_img, #trainee_profile_img1').change(function(){
        var file_name = $(this).val();
        var fileObj = this.files[0]; // get file object
        var calculatedSize = fileObj.size/(1024*1024); // in MB
        var split_extension = file_name.split(".");
        ...
        if(jQuery.inArray(split_extension[1].toLowerCase(), ext ) != -1 && calculatedSize < 10)
        {
            $("div.img-profile div#error_msg").hide();
            $('div.img-profile img#loading-img').show(); 
            $('#imagecrop').modal({show:true, backdrop: "static", keyboard: false});
        ...
        $("#crop").on("click",function(){
            var resp = CanvasCrop.getDataURL("jpeg");
            $.ajax({
                    url: "<?php echo $this->request->webroot; ?>trainees/cropImage",
                    type: "post",
                    data: {resp:resp},
                    dataType: "json",
    ...
    $(document).ready(function(){
        $('body').on('click','#skip-btn',function(){
            ...
            $('div.img-profile img#loading-img').hide(); 
            $('img#profile-img').attr('src','<?php echo $this->request->webroot; ?>uploads/trainee_profile/' + img_src);
            $("div.img-profile div#success_msg").html("<center><i class='fa fa-check'> Image Uploaded Successfully </i></center>").show();
            $("div.img-profile div#error_msg").hide();
            sessionStorage.removeItem('img_src');
        });
    });*/

</script>

<!-- Image Profile With Crop End -->
